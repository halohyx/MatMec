...
def test(
    ...
    **kwargs,
    ...
    if total_test < len(all_sys):
        all_sys = random.sample(all_sys, total_test)
...
detail_file = 'energy'
atomic = False
models = ['Ti5_attn_i.pb', 'Ti5_attn.pb']#, 'Ti5_big_i.pb', 'Ti5_big_i2.pb', 'Ti5_big_s.pb', 'Ti5_small_i.pb', 'Ti5_small_i2.pb', 'Ti5_small_s.pb']
system = 'data.iters'
total_test = 10000
...
for model in models:
    test(model=model, 
         ...
         rand_seed = None)
    output = detail_file + '.e.out'
    shutil.copy(output, "%s.e.out" % model)
    with open(output, 'r') as f:
        line = f.readline()
        energy_list = []
        while line:
            if line[0] != '#':
                energy_list.append(line.split())
            line = f.readline()
        energy_list = np.array(energy_list, dtype=float)
        with open("%s.e.plot" % model, 'w') as f:
            s = ''
